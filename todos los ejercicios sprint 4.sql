...
UPDATE transactions 
SET product_ids = REPLACE(product_ids, ' ', '');  -- CAMBIO EL ESPACIO POR NO ESPACIO ' ' POR ''
...
SELECT  id, name, surname, phone, email, birth_date, 'America' AS continent, country, city, postal_code, address 
FROM american_users
UNION ALL
SELECT  id, name, surname, phone, email, birth_date, 'Europe', country, city, postal_code, address 
FROM european_users;
...
SELECT user_id, name, surname, COUNT(user_id) AS "cantidad de transacciones"
FROM transactions
...
SELECT company_name,  iban, credit_cards.id AS credit_card_id, round(AVG(amount),2) AS media_gasto 
FROM transactions
...
SELECT subquery.card_id AS card_id, CASE WHEN SUM(subquery.declined) = 3 THEN 'inactivo'   -- si los declines suman 3 significa que los ultimos 3 estan en 1 por lo tanto es inactivo--
									ELSE 'activo'											-- si no suma 3 significa que o solo el ultimo esta declined o ha pasado un declined pero ahora ya ha vuelto a funcionar--		
									END AS state
FROM (SELECT card_id, timestamp, declined, ROW_NUMBER() OVER (PARTITION BY card_id ORDER BY timestamp DESC) AS rowi  -- coloco rownumber ordenador por fecha descendiente para luego filtra las ultimas 3 --
     FROM transactions
      ) AS subquery
WHERE subquery.rowi <= 3             -- aqui establezco que solo muestra las ultimas 3, osea row numer 1, 2, 3, los demas los descarto.--
GROUP BY subquery.card_id;

-- CREO LA FK PARA CREDIT_CARDS

ALTER TABLE credit_card_state
ADD CONSTRAINT fk_state_card
FOREIGN KEY (card_id) REFERENCES credit_cards(id)
ON UPDATE CASCADE
ON DELETE CASCADE,
ADD UNIQUE (card_id);
